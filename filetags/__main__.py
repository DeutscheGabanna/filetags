...
def handle_tag_gardening(vocabulary):
    ...
    if vocabulary:
        ...
        print_tag_dict(
            {key: value for key, value in only_similar_tags_by_alphabet_dict.items() if key not in vocabulary},
            vocabulary, sort_index=0, print_similar_vocabulary_tags=True)
        print_tag_dict({key: value for key, value in only_similar_tags_by_alphabet_dict.items() if key in vocabulary},
                       vocabulary, sort_index=0, print_similar_vocabulary_tags=True)
    else:
        print("\nTags which have similar other tags are probably typos or plural/singular forms of others:")
...
def locate_and_parse_controlled_vocabulary(startfile):
    ...
    if filename:
        logging.debug(
            'locate_and_parse_controlled_vocabulary: locate_file_in_cwd_and_parent_directories returned: ' + filename)
    else:
        logging.debug(
            ...
            for rawline in filehandle:
                if rawline.strip().lower().startswith(INCLUDE_PREFIX):
                    ...
                    line = rawline.strip().split('#')[
                        0].strip()  # split and take everything before the first '#' as new "line"
...
def get_upto_nine_keys_of_dict_with_highest_value(mydict, list_of_tags_to_omit=[],
                                                  omit_filetags_donotsuggest_tags=False):
    """
...
def generate_tagtrees(directory, maxdepth, ignore_nontagged, nontagged_subdir, link_missing_mutual_tagged_items,
    ...
    def create_tagtrees_dir(basedirectory, tagpermutation):
        ...
        current_directory = os.path.join(basedirectory,
                                         *[x for x in tagpermutation])  # flatten out list of permutations to elements
        ...
        if len(tags_of_currentfile) == 0:
            ...
            for currentdepth in range(1, maxdepth + 1):
                ...
                for tagpermutation in itertools.permutations(tags_of_currentfile, currentdepth):
                    ...
                    current_directory = os.path.join(directory, *[x for x in
                                                                  tagpermutation])  ## flatten out list of permutations to elements
                ...
                for unique_tagset in unique_tags:
                    ...
                    if not set(tags_of_currentfile).intersection(set(unique_tagset)):
                        ...
                        no_uniqueset_tag_found_dir = os.path.join(directory,
                                                                  'no-' + ("-").join(
                                                                      unique_tagset))  # example: "no-draft-final"
...
def all_files_are_links_to_same_directory(files):
    ...
    for current_file in files:
        ...
        current_link_components = split_up_filename(
            current_file)  # 0 = absolute path incl. filename; 1 = dir; 2 = filename
...
def main():
    ...
    if options.list_tags_by_alphabet or \
        ...
        if options.remove:
            ...
            if files:
                ...
                if len(files) == 1 and \
                    ...
                    original_file = split_up_filename(
                        get_link_source_file(files[0]))  # 0 = absolute path incl. filename; 1 = dir; 2 = filename
            ...
            logging.debug('derived vocabulary with %i entries' % len(
                vocabulary))  # using default vocabulary which was generate above
